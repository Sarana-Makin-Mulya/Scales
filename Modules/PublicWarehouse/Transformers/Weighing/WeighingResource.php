...
class WeighingResource extends Resource
{
    /**
    ...
    public function toArray($request)
    {
        return [
            ...
            'stage' =>$this->stage,
            'status' =>$this->status,
            'url_edit' => route('wh.weighing.update', [$this->id]),
            'url_delete' => route('ajax.wh.destroy.weighing', [$this->id]),
        ...
        if (!empty($weighing)) {
            if ($weighing->weighing_category_id==1) {
                ...
                $data = WeighingItems::query()
                    ...
                    ->first();
                $item_name = (!empty($data)) ? $data->name: 'Other';
            }
        }
